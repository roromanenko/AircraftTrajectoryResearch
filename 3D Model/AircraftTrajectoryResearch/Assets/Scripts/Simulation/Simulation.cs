...
public class Simulation
{
	...
	public SimulationResult Run(double dt, double w, double NV, int controlLawNumber, double X0, double Z0)
	{
		...
		y[8] = X0;
		y[9] = Z0;
		double Pzt0 = 57.3 * Math.Atan2(0 - Z0, 0 - X0);
		y[0] = -Pzt0;
		double t = 0;

		//Main loop
		while (y[8] <= 0 || (y[9] > 20 && y[8] < -20))
		{
			var coeffs = _coeff.ComputeCoefficients(_parameters, mass);
			var a = coeffs.A;
			var b = coeffs.B;

			DN = _parameters.k_omega_y * y[1];
			...
			x[0] = y[1];                                                                    //?` Psi`
			x[1] = -a[1] * x[0] - b[6] * x[2] - a[2] * x[5] - a[3] * DN - b[5] * DE;        //?`` Psi``
			x[2] = y[3];                                                                    //gamma`
			x[3] = -a[6] * x[0] - b[1] * x[2] - b[2] * x[5] - a[5] * DN - b[3] * DE;        //gamma``
			x[4] = x[0] + b[7] * x[2] + b[4] * y[2] - a[4] * x[5] - a[7] * DN;              //beta`
			x[7] = -y[0];                                                                   //psi_g
			x[11] = w * Math.Cos((NV - x[7]) * (Math.PI / 180.0));                          //Wx
			x[12] = w * Math.Sin((NV - x[7]) * (Math.PI / 180.0));                          //Wz
			x[6] = -57.3 * (x[12] / _parameters.V);                                         //beta_w
			x[5] = y[4] + x[6];                                                             //beta_v
			x[8] = (_parameters.V + x[11]) * Math.Cos((x[7] + y[4]) * (Math.PI / 180.0));   //X`
			x[9] = (_parameters.V + x[11]) * Math.Sin((x[7] + y[4]) * (Math.PI / 180.0));   //Z`
			x[10] = Math.Sqrt(Math.Pow(x[8], 2) + Math.Pow(x[9], 2));                       //V_sh
